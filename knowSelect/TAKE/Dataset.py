...
class Dataset(Dataset):
    ...
    def load(self):
        for id in range(len(self.episodes)):
            ...
            episode_content = {"context": [], "response": [], "knowledge_pool": [], "knowledge_piece_mask": [], "knowledge_label": [], 'Initiative_label': []}
            for id_in_episode, example in enumerate(episode):
                ...
                context_ = [CLS_WORD] + context
                if len(context_) >= self.context_len:
                    context_ = [CLS_WORD] + context_[-(self.context_len-2):] + [SEP_WORD]
                elif len(context_) < self.context_len:
                    context_ = context_ + [PAD_WORD] * (self.context_len -1 - len(context_)) + [SEP_WORD]
                ...
                # process for initiative label
                episode_content["Initiative_label"].append(torch.tensor([example['ID_label']], requires_grad=False).long())
                ...
                for pid in temp_sample_knowledge_pool:
                    ...
                    if len(p) >= self.knowledge_sentence_len:
                        p = context_ + p[:self.knowledge_sentence_len]
                    elif len(p) < self.knowledge_sentence_len:
                        p = context_ + p + [PAD_WORD] * (self.knowledge_sentence_len - len(p))
                    assert len(p) == self.knowledge_sentence_len + self.context_len
                ...
                while len(knowledge_text_list) < self.max_knowledge_pool:
                    knowledge_text_list.append(context_ + [CLS_WORD] + [PAD_WORD] * (self.knowledge_sentence_len - 2)+ [SEP_WORD])
            ...
            while len(episode_content["context"]) < self.max_episode_length:
                ...
                episode_content["response"].append(torch.tensor([self.vocab2id.get(w) if w in self.vocab2id else self.vocab2id[UNK_WORD] for w in ([CLS_WORD]+[PAD_WORD] * (self.max_dec_length-1))], requires_grad=False).long()) # [max_dec_length]
                ...
                episode_content['Initiative_label'].append(torch.tensor([0], requires_grad=False).long())
            ...
            assert len(episode_content["context"]) == len(episode_content["response"]) == len(
                episode_content["knowledge_pool"]) == len(episode_content["knowledge_piece_mask"]) == len(
                episode_content["knowledge_label"]) == len(episode_content["Initiative_label"]) == self.max_episode_length


            id_episode_tensor = torch.tensor([id]).long()
            ...
            Initiative_label_episode_tensor = torch.cat(episode_content["Initiative_label"])  # [max_episode_length]


            self.episode_tensor.append(
                [id_episode_tensor, context_episode_tensor, response_episode_tensor, knowledge_pool_episode_tensor,
                 knowledge_piece_mask_episode_tensor, knowledge_label_episode_tensor, Initiative_label_episode_tensor, episode_mask])
    ...
    def __getitem__(self, index):
        ...
        return [episode[0], episode[1], episode[2], episode[3], episode[4], episode[5], episode[6], episode[7]]
...
def collate_fn(data):
    id_episodes, context_episodes, response_episodes, knowledge_pool_episodes, knowledge_piece_mask_episodes, knowledge_label_episodes, Initiative_label_episodes, episode_mask_episodes = zip(*data)
    ...
    return {'episode_id': torch.cat(id_episodes),  # [batch]
            ...
            'Initiative_label': torch.stack(Initiative_label_episodes),  # [batch, max_episode_length]
