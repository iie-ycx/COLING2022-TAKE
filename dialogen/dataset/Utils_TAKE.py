    ...
    with codecs.open(file, encoding='utf-8') as f:
        for line in f:
            ...
            if temp[0] in poolset:
                passage[temp[0]] = temp[1] #' . '.join([' '.join(tokenizer(sent)) for sent in nltk.sent_tokenize(temp[1])]).split(' ')  # list的形式
...
def load_default(dataset, answer_file, passage_file, pool_file, qrel_file, query_file, tokenizer=None, topk=None, randoms=1):
